...
sys.path.append("../") #/opt/nvidia/deepstream/deepstream-5.1/workspace/deepstream_python_apps/apps/
# sys.path.append("/opt/nvidia/deepstream/deepstream-5.1/workspace/deepstream_python_apps/apps/") 
import gi
import configparser
import argparse
...
# This adds a console handler that outputs to stdout
...
def on_debug(category, level, dfile, dfctn, dline, source, message, user_data):
    if source:
        logging.info('{}[{}]: {}'.format(
            Gst.DebugLevel.get_name(level), source, message.get()))
    else:
        logging.info('{}: {}'.format(
            Gst.DebugLevel.get_name(level), message.get()))


def set_gst_logger(enable=False , verbosity=3):
    '''
    GST logger configuration
    Verbosity leveles are : 0-None < 1.ERROR < 2.WARNING < 3.FIXME < 4.INFO < 5.DEBUG < 6.LOG < 7.TRACE < 9.MEMDUMP
    Always recommended to use verbosity upto 2. Others wise huge verbose logs from GST will freeze the current system
    '''
    result = True
    try:
        if enable:
            ...
            if verbosity and isinstance(verbosity, int) and (verbosity in range(0,7)):
                ...
                result = False
                logging.error("GST verbosity allowed to be in 0-6, chosing 1 as default")
...
def parse_args():
    stream_paths = ["rtsp://192.168.1.9:8555/video1"] # ,'file:///opt/nvidia/deepstream/deepstream-6.3/sources/deepstream_python_apps/2.mp4'
    # stream_paths = ["file:///opt/nvidia/deepstream/deepstream-5.1/workspace/1.mp4","file:///opt/nvidia/deepstream/deepstream-5.1/workspace/2.mp4"]
                    # "file:///opt/nvidia/deepstream/deepstream-5.1/workspace/2.mp4"]  # rtsp://192.168.1.4:8555/video
    stream_paths = ['file:///opt/nvidia/deepstream/deepstream-6.3/samples/streams/sample_1080p_h264.mp4']
    stream_paths = ['rtsp://192.168.1.13:8555/video1']
    return stream_paths


if __name__ == "__main__":
    set_gst_logger(enable=True,verbosity=5)
